    ...
    switch (currentView) {
      ...
      case 'notes-view':
        return (
          <NotesView 
            notes={notesSource}
            onSaveNotes={activeProfile ? null : (val) => setNotes(val)}
          />
        );
      case 'todo-view':
        return (
          <TodoView 
            todos={todosSource}
            role={activeProfile ? 'visitor' : role}
            onToggleTodo={activeProfile ? null : (todoId) => setTodos(todos.map(t => t.id === todoId ? { ...t, completed: !t.completed } : t))}
            onAddTodo={activeProfile ? null : (text) => setTodos([...todos, { id: `todo-${Date.now()}`, text, completed: false }])}
            onDeleteTodo={activeProfile ? null : (todoId) => setTodos(todos.filter(t => t.id !== todoId))}
          />
        );
      case 'dvd-rack-view':
        return (
    ...
    <div className="modal-overlay active" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="modal-content" style={{ maxWidth: '400px', width: '90%' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '12px' }}>
          ...
          <div className="form-group" style={{ textAlign: 'left' }}>
            <label className="form-label" style={{ fontWeight: 'bold', fontSize: '12px' }}>好友解鎖金鑰 (看隱密貼文)</label>
            <input type="text" className="form-input" value={friendKey} onChange={(e) => setFriendKey(e.target.value)} required />
          </div>
